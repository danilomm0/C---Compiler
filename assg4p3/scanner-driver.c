...
{
    if (chk_decl_flag && found->type != FUNC)
    {
...
int parse()
{
    SymTab *println = add_symtab(0, "println", FUNC);
    println->args = 1;
    current = get_token();
    while (current != EOF)
    {
