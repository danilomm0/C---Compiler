...
void gen_boolexp(AST *ast, Quad *yes, Quad *no)
{
    ...
    gen_3addr(ast->sub0);
    gen_3addr(ast->sub1);
    Quad *inst1 = make_quad(type, make_op(0, ast->sub0->loc, NULL), make_op(0, ast->sub1->loc, NULL), make_op(0, NULL, yes));
    Quad *inst2 = make_quad(GOTO, NULL, NULL, make_op(0, NULL, no));
...
void gen_3addr(AST *ast)
{
    ...
    {
        AST *temp;
        Quad *head = NULL, *tail = NULL, *param, *call;
        gen_3addr(ast->sub0);
    ...
    else if (ast->type == INTCONST)
    {
        ...
        SymTab *stemp = add_symtab(1, strdup(name), LOCAL);
...
{
    assert(qtemp != NULL);
    char *names[] = {"no_op", "move", "goto", "if_eq", "if_ne", "if_le", "if_lt", "if_ge", "if_gt", "label", "enter", "leave", "param", "call", "ret", "retrieve"};
